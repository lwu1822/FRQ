...
*/
    ...
    */
    ...
    @GetMapping("all/**")
    public ResponseEntity<JsonNode> allDirectories(HttpServletRequest request) throws JsonMappingException, JsonProcessingException {

        String test = request.getRequestURI().split(request.getContextPath() + "/all/")[1];
      
         Calculator calculator_obj = new Calculator(test);
      // Turn Year Object into JSON
      ObjectMapper mapper = new ObjectMapper(); 
      JsonNode json = mapper.readTree(calculator_obj.toStringJson()); // this requires exception handling

      return ResponseEntity.ok(json); 
        //return request.getRequestURI().split(request.getContextPath() + "/all/")[1];
    }
